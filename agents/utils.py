...
class WeatherGenerator:
    def __init__(
        ...
        lon: float = 6.4794,
        lat: float = 50.8037,
        path_to_weather_file: str = r"./weather_2022.csv",
    ):
    ...
    def get_weather(self, date: pd.Timestamp):
        ...
        weather = dict(
            ...
            wind_speed=self.weather.loc[
                self.weather.index.date == date.date(), "wind_speed"
            ].values,
            temp_air=self.weather.loc[
                self.weather.index.date == date.date(), "temp_air"
            ].values,
        )
