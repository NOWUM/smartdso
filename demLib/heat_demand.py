...
BUILDINGS = pd.read_csv(
    r"./demLib/data/heat_demand/buildings.csv", sep=";", index_col=0
)
...
HEAT_DEMAND = gpd.overlay(
    HEAT_DEMAND,
    gpd.GeoDataFrame(bounding_box, columns=["geometry"]),
    how="intersection",
)
...
for consumer_id, coord in tqdm(coords.iterrows(), total=coords.shape[0]):
    ...
    rows = HEAT_DEMAND.loc[
        HEAT_DEMAND["geometry"].apply(lambda x: x.contains(Point((lon, lat))))
    ]
    if rows.empty:
        distance = HEAT_DEMAND["geometry"].apply(
            lambda x: x.distance(Point((lon, lat)))
        )
