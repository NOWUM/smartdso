...
REFERENCE_TEMPERATURE = pd.DataFrame(
    data=dict(temp_air=REFERENCE_TEMPERATURE),
    index=pd.date_range(
        start="2018-01-01", freq="h", periods=len(REFERENCE_TEMPERATURE)
    ),
)
...
class StandardLoadProfile:
    def __init__(
        self,
        demandQ: float,
        building_parameters: tuple = (2.8, -37.5, 5.4, 0.17),
        resolution: int = 96,
    ):
        self.demandQ = demandQ
        self.building_parameters = building_parameters
        self.day_values = (
            REFERENCE_TEMPERATURE.groupby(REFERENCE_TEMPERATURE.index.day_of_year)
            .mean()
            .values.flatten()
        )
    ...
    def run_model(self, temperature: np.array = None):
        ...
        temperature = (mean_temperature + 0.5 * t1 + 0.25 * t2 + 0.125 * t3) / (
            1 + 0.5 + 0.25 + 0.125
        )
