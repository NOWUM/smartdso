...
base_scenarios = [
    "F-PlugInCap-PV80-PriceFlat",
    "F-MaxPvCap-PV80-PriceFlat",
    "F-MaxPvCap-PV80-PriceSpot",
    "F-MaxPvSoc-PV80-PriceSpot",
]
pv_scenarios = [
    scenario
    for scenario in getter.scenarios
...
def get_comparison(car_id: str = "S2C122") -> (pd.DataFrame, dict):
    ...
    market_price = getter.get_mean_values(
        scenario=sc, parameter="market_prices", date_range=date_range
    )
    availability = 100 * getter.get_mean_values(
        scenario=sc, parameter="availability"
    ).mean(axis=1)
    pv_generation = 100 * getter.get_mean_values(
        scenario=sc, parameter="residual_generation"
    ).mean(axis=1)
...
def get_impact_grid_level(scenarios: list):
    transformer_impact = {
        sc: getter.get_aggregated_utilization_values(
            sc, func="max", asset="transformer"
        )
        for sc in scenarios
    }
...
def get_time_utilization(sub_id: int = 5):
    ...
    for sc in base_scenarios:
        ...
        for iteration in range(10):
            val = getter.get_aggregated_utilization_values(
                scenario=sc, func="mean", asset="transformer", iteration=iteration
            )
            values.append(val[sub_id].values)
        values = np.asarray(values)
        utilization[sc] = (
            values.max(axis=0, initial=-np.inf),
            values.min(axis=0, initial=np.inf),
            values[0, :],
        )
...
def get_economic_table(scenarios: list):
    table = pd.DataFrame(
        ...
        index=[
            ...
            "grid [%]",
            "mean grid fee [€/kWh]",
            "grid fee 95 % quantile",
            "grid fee mean > 95 % quantile",
            "mean utilization [%]",
            "utilization 95 % quantile",
            "utilization mean > 95 % quantile",
        ],
    )
    ...
    for sc in scenarios:
        ...
        market_prices = getter.get_mean_values(
            parameter="market_prices", scenario=sc, date_range=sim_range
        )
        ...
        table.at["total charging [MWh]", sc] = round(
            np.mean(r["total_charged"]) / 1e3, 2
        )
...
def export_strategy_impact_on_transformer_level():
    ...
    fig.savefig(
        r"./eval/plots/strategy_impact_on_transformer_pv.svg", bbox_inches="tight"
    )
...
def export_pv_impact_on_grid_level():
    ...
    fig = plotter.plot_pv_impact_grid_level(
        t_impact, total_charged, getter.pv_capacities
    )
