...
class EvalGetter:
    def __init__(self, database_uri: str = DATABASE_URI):
        ...
        self._total_consumers["sub_grid"] = [
            grid_series.loc[node] if node in grid_series.index else -1
            for node in self._total_consumers["bus0"].values
        ]
        self._total_consumers["residents"] = self._total_consumers["jeb"].apply(
            lambda x: int(min(max(x / 1500, 1), 2))
        )
        self._sub_grids = self._cp.mapper.unique()

        self._benefit_functions = pd.read_csv(
        ...
        def get_scenarios():
            query = (
                "select distinct scenario from electric_vehicle "
        ...
        def get_cars(scenario: str):
            query = (
                f"select distinct id_ from electric_vehicle where scenario='{scenario}' "
        ...
        def get_time_range(scenario: str):
            query = (
                f"select distinct time from grid_summary where scenario='{scenario}' and iteration = 0"
                f"order by time"
            )
            data = pd.read_sql(query, self.engine)
        ...
        def get_pv_capacity():
            ...
            for sub_id in self._sub_grids:
                total_residents = consumers.loc[
                    consumers["sub_grid"] == sub_id, "residents"
                ].sum()
    ...
    def get_all_utilization_values(self, scenario: str, asset: str = "transformer"):
        table = "grid_asset"
        query = (
            f"select utilization from {table} "
    ...
    def get_mean_values(
        self, parameter: str, scenario: str, date_range: pd.DatetimeIndex = None
    ):
    ...
    def get_ev(self, scenario: str, ev: str):
        query = (
            f"select time, avg(usage) as usage, avg(final_charging) as charging, "
    ...
    def get_aggregated_utilization_values(
        self,
        scenario: str,
        iteration: int = None,
        func="max",
        asset: str = "transformer",
    ) -> pd.DataFrame:
    ...
    def get_typ_values(
        ...
        query = (
            f"select to_char(time, 'hh24:mi') as interval, {insert}"
