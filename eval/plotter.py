...
class EvalPlotter:
    def __init__(self, width: float = 9.05, offset_days=1):
        self.colors = {
            "pv_generation": (230 / 255, 230 / 255, 74 / 255, 0.65),
    ...
    def plot_charging_compare(self, data: dict, car_id: str):
        ...
        plt_data = plt.subplots(
            len(data),
            1,
            sharex="all",
            figsize=(self._width * cm, 1.5 * self._width * cm),
            dpi=300,
        )
        ...
        for plot, values in zip(plots[0], data.items()):
            ...
            plot.fill_between(
                val.index,
                val["used_pv_generation"].values,
                color=self.colors["used_pv_generation"],
                linewidth=0,
            )
            ...
            sec_plot.plot(
                val.index,
                val["soc"].values * 100,
                color=self.colors["soc"],
                linewidth=1,
            )
            sec_plot.fill_between(
                val.index,
                100 * val["usage"].values,
                color=self.colors["usage"],
                linewidth=0.25,
            )
    ...
    def plot_impact(self, data: dict, pv_colors: bool = False, linewidth: float = 0.5):
        fig, (ax, ax_zoom) = plt.subplots(
            1,
            2,
            figsize=(self._width * cm, self._width / 2 * cm),
            dpi=300,
        ...
        for name, val in data.items():
            if pv_colors:
                pv = name.split(" ")[-1]
                ax.plot(
                    100 * np.arange(num) / num,
                    val.values,
                    linewidth=linewidth,
                    color=self.pv_colors[pv],
                )
                val = val[: int(len(val) * 0.2)]
                ax_zoom.plot(
                    100 * np.arange(len(val)) / num,
                    val.values,
                    linewidth=linewidth,
                    color=self.pv_colors[pv],
                )
                ...
                val = val[: int(len(val) * 0.2)]
                ax_zoom.plot(
                    100 * np.arange(len(val)) / num, val.values, linewidth=linewidth
                )
        ...
        for name, val in data.items():
            ...
            if pv_colors:
                pv = name.split(" ")[-1]
                ax_zoom.scatter(
                    [1], p99_quantile, marker="o", s=1, color=self.pv_colors[pv]
                )
    ...
    def plot_impact_grid_level(
        self, transformer_data: dict, total_charged: dict, pv_ratio: dict
    ):
        data = pd.DataFrame.from_dict(pv_ratio, orient="index").sort_index()
        pv_ratio = data.values

        fig, (ax_grid, ax_transformer) = plt.subplots(
            2,
            1,
            sharex="all",
            gridspec_kw={"height_ratios": [2, 8]},
            figsize=(self._width * cm, self._width * cm),
            dpi=300,
        )

        grids = [*transformer_data.values()][0].columns
        ax_grid.plot(
            [int(i) for i in data.index], pv_ratio[:, 0], linewidth=0.5, color="black"
        )
        ax_grid.plot(
            [int(i) for i in data.index], pv_ratio[:, 1], linewidth=0.5, color="blue"
        )
        ax_grid.grid(True)
        ...
        for grid in grids:
            ...
            for i in range(len(trans_values)):
                ax_transformer.scatter(
                    grid,
                    trans_values[i],
                    marker=marker[i],
                    color=self.sub_colors[grid],
                    s=4,
                )
    ...
    def plot_pv_impact_grid_level(
        self, transformer_data: dict, total_charged: dict, pv_ratio: dict
    ):
        data = pd.DataFrame.from_dict(pv_ratio, orient="index").sort_index()
        pv_ratio = data.values

        fig, (ax_grid, ax_25, ax_transformer) = plt.subplots(
            3,
            1,
            sharex="all",
        ...
        for grid in grids:
            ...
            max_utilization = utilization_values.loc[
                utilization_values.values > q95, grid
            ].mean()
            max_utilization_values[grid] = max_utilization
            ax_25.scatter(
                grid, max_utilization, marker="^", color=self.sub_colors[grid], s=4
            )
    ...
    def plot_grid(self):
        fig, ax = plt.subplots(
            1, 1, sharex="all", figsize=(self._width * cm, self._width * cm), dpi=300
        )
        ...
        for grid in self._all_nodes["sub_grid"].unique():
            ...
            for idx, values in lines.iterrows():
                ax.plot(
                    eval(values.lon_coords),
                    eval(values.lat_coords),
                    color="black",
                    linewidth=1,
                )
    ...
    def plot_overview(self, data: pd.DataFrame):
        fig, ax = plt.subplots(
            1, 1, figsize=(self._width * cm, 0.5 * self._width * cm), dpi=300
        )
        ...
        ax.plot(
            data.index,
            data["price"].values,
            color=self.colors["market_price"],
            linewidth=1,
        )
    ...
    def plot_benefit_function(self, data: pd.DataFrame):
        fig, ax = plt.subplots(
            1, 1, figsize=(self._width * cm, self._width * cm), dpi=300
        )
        ax.grid(True)
        ax.set_xlabel("SoC [%]")
