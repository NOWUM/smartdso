...
def get_plot(
    ...
    def get_edges(v_level: float = 0.4) -> go.Scattermapbox:
        ...
        for name, attributes in lines.iterrows():
            ...
            lat_coords, lon_coords = np.append(lat_coords, [None]), np.append(
                lon_coords, [None]
            )
            names = np.append(names, [None])

        grid_plot = go.Scattermapbox(
            name=f"lines",
            hoverinfo="text",
            hovertext=names,
            mode="lines",
            lon=lon_coords,
            lat=lat_coords,
            line=dict(width=2, color=COLOR[str(v_level)]),
        )
        return grid_plot
    ...
    def get_nodes(v_level: float = 0.4) -> go.Scattermapbox:
        ...
        grid_plot = go.Scattermapbox(
            ...
            lat=busses["lat"],
            hoverinfo="text",
            hovertext=[f"<b>Name:</b> {index}" for index in busses.index],
    ...
    def get_consumers(v_level: float = 0.4) -> go.Scattermapbox:
        ...
        grid_plot = go.Scattermapbox(
            name=f"Consumers on {v_level} kV",
            mode="markers",
            lon=demand["lon"],
            lat=demand["lat"],
            hoverinfo="text",
            hovertext=[f"<b>Name:</b> {index}" for index in demand.index],
        )
        return grid_plot
        ...
        fig.update_layout(
            mapbox=dict(
                ...
                center=go.layout.mapbox.Center(
                    lat=nodes["lat"].mean(), lon=nodes["lon"].mean()
                ),
            ),
            autosize=True,
        )
