...
class HouseholdModel(BasicParticipant):
    ...
    def __init__(self, residents: int,
        ...
        self.cars = {key_generator(pv_capacity, person.car.capacity): person.car for person in self.persons if
                     person.car.type == 'ev'}
    ...
    def _optimize_photovoltaic_usage(self, d_time: datetime, strategy: str = 'required'):
        ...
        demand = {key: car.get(CarData.demand, slice(d_time, None)).values[steps] for key, car in self.cars.items()}
        # -> get prices
        ...
        for key, car in self.cars.items():
            for t in self.t:
                ...
                if t > 0:
                    self._model.soc_limit.add(self._model.volume[key, t] == self._model.volume[key, t-1] + balance)
                else:
